...
*   beta distribution, gamma distribution, erlang distribution, uniform,
...
*   DATE:   22 April 2004
*   UPDATE: 21 November 2006, 19 December 2006
*
*   DOCUMENTATION:
*   See Michael Thomas Flanagan's Java library on-line web page:
*   PsRandom.html
*
*   Copyright (c) April 2004, November 2006, December 2006 Michael Thomas Flanagan
*
*   PERMISSION TO COPY:
   ...
   public double[] rayleighArray (double sigma, int n){
    ...
    public double nextMinimalGumbel(double mu, double sigma){
        ...
        if(this.methodOptionDecimal==1){
            ...
            ran = Math.log(Math.log(1.0D/(1.0D-this.nextDouble())))*sigma+mu;
        }
        return ran;
    }
    ...
    public double[] nextLogistic(double mu, double scale, int n){

        double[] ran = new double[n];
        if(this.methodOptionDecimal==1){
            for(int i=0; i<n; i++){
                ran[i] = 2.0D*scale*Fmath.atanh(2.0D*rr.nextDouble() - 1.0D) + mu;
            }
        }
        else{
            for(int i=0; i<n; i++){
    ...
    // Returns a Logistic distribution random deviate
    // mu = location parameter, scale = scale parameter
    public double nextLogistic(double mu, double scale){
        double ran = 0.0D;
        if(this.methodOptionDecimal==1){
            ran = 2.0D*scale*Fmath.atanh(2.0D*rr.nextDouble() - 1.0D) + mu;
        }
        else{
            ran = 2.0D*scale*Fmath.atanh(2.0D*this.nextDouble() - 1.0D) + mu;
        }
        return ran;
    }
    ...
    // Returns a Student's t random deviate
    // nu  =  the degrees of freedom
    ...
    public double[] betaArray(double min, double max, double alpha, double beta, int n){
        ...
        for(int i=0; i<n; i++){
            ...
            // call root searching method, bisectNewtonRaphson
            ran[i] = realR.bisect(bart, lowerBound, upperBound);
        }
    ...
    public double[] gammaArray(double mu, double beta, double gamma, int n){
        ...
        for(int i=0; i<n; i++){
            ...
            // call root searching method, bisectNewtonRaphson
            ran[i] = realR.bisect(gart, lowerBound, upperBound);
        }
